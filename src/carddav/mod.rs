...
/// CardDAV client with a runtime-bound connection pool.
///
/// Keep a client on the Tokio runtime where it was constructed. A client constructed
/// outside a runtime must be used on only one runtime. For multiple runtimes,
/// construct separate clients inside each runtime, including any [`crate::remote::HttpServer`].
...
<d:propfind xmlns:d="DAV:" xmlns:card="urn:ietf:params:xml:ns:carddav">
  <d:prop>
    ...
    pub async fn list_contacts(&self, addressbook_href: &str) -> Result<Vec<Contact>> {
        ...
        let response = self.query_addressbook(addressbook_href).await?;
        let status = response.status();
        let text = read_response(response).await?;
    ...
    async fn query_addressbook(&self, href: &str) -> Result<reqwest::Response> {
...
<card:addressbook-query xmlns:d="DAV:" xmlns:card="urn:ietf:params:xml:ns:carddav">
  <d:prop>
    ...
    fn contact_at(&self, vcard: &str, href: &str) -> Option<Contact> {
        let mut contact = parse_vcard(vcard)?;
    ...
    async fn find_contact_href(&self, contact_id: &str) -> Result<Option<ContactResource>> {
        ...
        for ab in addressbooks {
            let response = self.query_addressbook(&ab.href).await?;
...
fn property_label(line: &str) -> Option<String> {
    line.split("TYPE=")
        .nth(1)
...
fn build_vcard(
    ...
    if let Some((given, family)) = name.split_once(' ') {
        lines.push(format!(
            "N:{};{};;;",
            escape_vcard_text(family),
            escape_vcard_text(given)
        ));
    } else {
